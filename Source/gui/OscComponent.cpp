...
OscComponent::OscComponent (juce::String name,
                            juce::AudioProcessorValueTreeState& apvts,
                            ParamIdPrefix param_id_prefix)
    : SynthModuleComponent (name),
      frequency_slider_ ("Coarse"),
      ...
      volume_slider_ ("Level")
{
    ...
    fine_pitch_slider_.attatchToParameter (
        apvts, param_id_string_map.at (param_id_prefix) + "FREQUENCY_FINE");
    addAndMakeVisible (fine_pitch_slider_);

    addAndMakeVisible (frequency_modulation_);

    waveform_slider_.attatchToParameter (apvts,
                                         param_id_string_map.at (param_id_prefix) + "WAVEFORM");
    addAndMakeVisible (waveform_slider_);

    addAndMakeVisible (waveform_modulation_);

    volume_slider_.attatchToParameter (apvts, param_id_string_map.at (param_id_prefix) + "VOLUME");
...
void OscComponent::resized()
{
    ...
    waveform_slider_.setBounds (MODULE_PADDING + SLIDER_COMPONENT_WIDTH * 2 + MOD_COMPONENT_WIDTH,
                                ...
                                SLIDER_COMPONENT_WIDTH,
                                SLIDER_COMPONENT_HEIGHT);
    waveform_modulation_.setBounds (MODULE_PADDING + SLIDER_COMPONENT_WIDTH * 3
                                        + MOD_COMPONENT_WIDTH,
                                    MODULE_PADDING,
                                    MOD_COMPONENT_WIDTH,
                                    MOD_COMPONENT_HEIGHT);
    volume_slider_.setBounds (MODULE_PADDING + SLIDER_COMPONENT_WIDTH * 3 + MOD_COMPONENT_WIDTH * 2,
                              MODULE_PADDING,
                              SLIDER_COMPONENT_WIDTH,
                              SLIDER_COMPONENT_HEIGHT);
}
