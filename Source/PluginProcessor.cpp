...
void Mhj01AudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi_messages)
{
...
void Mhj01AudioProcessor::processOscs (SynthVoice& voice, double lfo1_mod, double lfo2_mod)
{
    ...
    double osc2_frequency_mod = juce::jmap<double> (
        lfo1_mod * getModWheelAmount ("MOD_WHEEL_OSC_2_LFO_1") * apvts.getRawParameterValue ("OSC_2_FREQUENCY_MOD_LFO_1")->load() + lfo2_mod * apvts.getRawParameterValue ("OSC_2_FREQUENCY_MOD_LFO_2")->load() + osc2_lfo2_aftertouch,
        0.0,
        2.0);
    ...
    double osc1_waveform_mod = juce::jmap<float> (
        lfo1_mod * apvts.getRawParameterValue ("OSC_1_WAVEFORM_MOD_LFO_1")->load() + lfo2_mod * apvts.getRawParameterValue ("OSC_1_WAVEFORM_MOD_LFO_2")->load(),
        0.0f,
        2.0f);
    double osc2_waveform_mod = juce::jmap<float> (
        lfo1_mod * apvts.getRawParameterValue ("OSC_2_WAVEFORM_MOD_LFO_1")->load() + lfo2_mod * apvts.getRawParameterValue ("OSC_2_WAVEFORM_MOD_LFO_2")->load(),
        0.0f,
        2.0f);
    voice.setWaveform (apvts.getRawParameterValue ("OSC_1_WAVEFORM")->load() * getModWheelAmount ("MOD_WHEEL_OSC_1_WAVEFORM") + osc1_waveform_mod + getChannelPressureAmount ("AFTERTOUCH_OSC_1_WAVEFORM"),
                       apvts.getRawParameterValue ("OSC_2_WAVEFORM")->load() * getModWheelAmount ("MOD_WHEEL_OSC_2_WAVEFORM") + osc2_waveform_mod + getChannelPressureAmount ("AFTERTOUCH_OSC_2_WAVEFORM"),
...
void Mhj01AudioProcessor::processFilter (SynthVoice& voice, double lfo1_mod, double lfo2_mod)
{
    ...
    float cutoff_mod = juce::jmap<float> (
        lfo1_mod * apvts.getRawParameterValue ("FILTER_CUTOFF_MOD_LFO_1")->load() * getModWheelAmount ("MOD_WHEEL_FILTER_LFO_1") + lfo2_mod * apvts.getRawParameterValue ("FILTER_CUTOFF_MOD_LFO_2")->load() + filter_lfo2_aftertouch + getChannelPressureAmount ("AFTERTOUCH_FILTER_CUTOFF"),
        0.0f,
        6500.0f);
    voice.setVcfParameters (apvts.getRawParameterValue ("FILTER_CUTOFF")->load() * getModWheelAmount ("MOD_WHEEL_FILTER_CUTOFF") + cutoff_mod,
...
juce::AudioProcessorValueTreeState::ParameterLayout Mhj01AudioProcessor::createParameters()
{
    ...
    parameters.push_back (std::make_unique<juce::AudioParameterFloat> ("OSC_1_FREQUENCY", "Frequency", 0.0f, 1.0f, 0.5f));
    parameters.push_back (std::make_unique<juce::AudioParameterFloat> ("OSC_1_FREQUENCY_FINE", "Frequency", -0.5f, 0.5f, 0.0f));
    parameters.push_back (std::make_unique<juce::AudioParameterFloat> ("OSC_1_FREQUENCY_MOD_LFO_1", "LFO 1 depth", 0.0f, 1.0f, 0.0f));
    parameters.push_back (std::make_unique<juce::AudioParameterFloat> ("OSC_1_FREQUENCY_MOD_LFO_2", "LFO 2 depth", 0.0f, 1.0f, 0.0f));
    ...
    parameters.push_back (std::make_unique<juce::AudioParameterFloat> ("FILTER_CUTOFF_MOD_LFO_2", "LFO 2 depth", 0.0f, 1.0f, 0.0f));
